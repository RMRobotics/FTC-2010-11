...
void processWheelMovement(){
  ...
  forwardLeft=(joystick.joy1_y2+joystick.joy1_x2)/2;
  ...
  if(joy1Btn(12)==1){//copyrighted PULSE feature by Max B. 2011
    if(!btn12Down){
...
void processScoringMovement(){
  if(scoreState==0){//in middle happy
    if(joy1Btn(5)==1){
      btn5Down=true;
      if(joy1Btn(7)==1){//confirmation for stationary dispensal
        wait10Msec(50);
        scoreOnStat();
        btn5Down=false;
      }
  ...
  if(scoreState==1){//about to dispense to rolling goal
    if(joy1Btn(5)==1){//cancel
      wait10Msec(50);
      moveScoreBoxTo(SERVO_SCORE_BOX_TILT_MID);
      scoreState=0;
    }
    if(joy1Btn(7)==1){//score
      wait10Msec(50);
      openScoreBoxDoor();
      scoreState=2;
    }
  ...
  if(scoreState==2){//dispensing now
    if(joy1Btn(7)==1||joy1Btn(5)==1){//close
      wait10Msec(50);
      closeScoreBoxDoor();
      scoreState=1;
    }
